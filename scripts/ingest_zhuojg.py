...
We use the *calligrapher-organized* half of the dataset as the primary source
because its folder names encode script style + author/work, e.g.
`楷-柳公权`, `行-米芾`, `楷-多宝塔`. The *character-organized* half has no
style or author metadata, so ingesting it would just pollute the style tabs.
We skip it here.

Folder-name decoding:
- `楷-柳公权`  -> style=kai, kind=calligrapher, name=柳公權
- `行-米芾`    -> style=xing, kind=calligrapher, name=米芾
- `楷-多宝塔`  -> style=kai, kind=work, name=多寶塔碑, author=顏真卿
- `楷-魏碑`    -> style=kai, kind=work, name=魏碑, author=NULL

Inner layout: two variants seen in the wild. The script auto-detects.
- subdir: `{style}-{name}/{char}/{file}.gif`
- flat:   `{style}-{name}/{char}_{seq}.gif`  (character is the first CJK
...
try:
    ...
    def _s2t_convert(s: str) -> str:  # type: ignore
        return s
...
# Simplified folder name -> (traditional name, dynasty) for the 19 zhuojg
# calligraphers we know about. Hand-mapped so we don't need a full s2t lib
# for author names.
...
# Simplified folder NAME (the part after "{style}-") -> (traditional work
# name, traditional author name or None). The author name must either match
...
# CJK Unified Ideographs (BMP) + Extension A. Covers the vast majority of
...
def main() -> int:
    ...
    parser.add_argument(
        "--calligrapher-dir",
        required=True,
        help="Path to the calligrapher-organized dataset root "
        "(parent of the 楷-柳公权/, 行-米芾/, ... folders)",
    )
    parser.add_argument(
        "--char-dir",
    ...
    missing_styles = set(STYLE_CHAR_TO_SLUG.values()) - style_id_by_slug.keys()
    if missing_styles:
        print(
            f"error: script_styles table is missing slugs {missing_styles}. "
            f"Run `npx tsx scripts/seed_reference.ts` first."
        )
        return 1

    if args.clean_source and not args.dry_run:
        cursor.execute(
            "DELETE FROM calligraphy_images WHERE source = ?", (args.source_tag,)
        )
        deleted = cursor.rowcount
        conn.commit()
        print(f"cleaned {deleted} existing rows with source={args.source_tag!r}")

    output_dir = Path(args.output_dir)

    # Pre-fetch the set of image_paths already present for this source, so
    # without --clean-source we can skip re-ingesting the same files.
    cursor.execute(
    ...
    for d in raw_folders:
        ...
        if not slug:
            print(f"skip {d.name}: unknown style char {style_char!r}")
            continue
        classified = classify_folder(name)
        if classified is None:
            print(
                f"skip {d.name}: unknown folder name {name!r} "
                f"(not in CALLIGRAPHER_S2T or WORK_FOLDERS)"
            )
            continue
